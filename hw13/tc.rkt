...
(: compile-body : (Listof (ENV -> VAL)) -> ENV -> VAL)
;; takes a list of expressions evaluates a list of expressions,
;; returns a single compiled expression function that runs them one by one
;; and returns the last value
(define (compile-body compiled-exprs)
  (: run-compiled : (Listof (ENV -> VAL)) ENV -> VAL)
  (define (run-compiled cexprs env)
    (let ([1st  ((first cexprs) env)]
          [rest (rest cexprs)])
      (if (null? rest)
          1st
          (run-compiled rest env))))
  (lambda ([env : ENV])
    (run-compiled compiled-exprs env))
...
(: compile : TOY -> ENV -> VAL)
;; compiles TOY expressions and
;; returns a callable function which can be used for evaluation
(define (compile expr)
  ...
  (cases expr
    ...
    [(Bind names exprs bound-body)
     (define compiled-body (map compile bound-body))
     (define compiled-exprs (map compile exprs))
     (lambda ([env : ENV])
       ((compile-body compiled-body) (extend names (map (caller env) compiled-exprs) env)))]
    [(BindRec names exprs bound-body)
     (define compiled-body (map compile bound-body))
     (define compiled-exprs (map compile exprs))
     (lambda ([env : ENV])
       ((compile-body compiled-body) (extend-rec names compiled-exprs env)))]
    ...
    [(Call fun-expr arg-exprs)
     (lambda ([env : ENV])
       (define fval ((compile fun-expr) env))
       ;; delay evaluating the arguments
       (define arg-vals (lambda () (map (caller env) arg-exprs)))
       (cases fval
    ...
    [(If cond-expr then-expr else-expr)
     (define compiled-cond-expr (map compile cond-expr))
     (define compiled-then-expr (map compile then-expr))
     (define compiled-else-expr (map compile else-expr))
     (lambda ([env : ENV])
       (if (cases (compiled-cond-expr env)
             [(RktV v) v] ; Racket value => use as boolean
             [else #t])   ; other values are always true
           (compiled-then-expr env)
           (compiled-then-expr env)))]))
