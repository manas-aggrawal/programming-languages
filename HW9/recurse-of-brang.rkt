     ...
     (match sexpr
       ...
       [(list 'rec (list (symbol: name) named) body)
        (let ([parsed-named (parse-sexpr named)])
          (if (cases parsed-named [(Fun params body) #t] [else #f])
              (WRec name parsed-named (parse-sexpr body))
              (error 'parse-sexpr "non-fun form in `rec'" sexpr)))]
...
;; testing parse-expr
(test (parse-sexpr "{rec {fact {fun {n}{if0 n 1 {* n {call fact {- n 1}}}}}}{call fact 5}}") =error> "non-fun form in `rec'")
