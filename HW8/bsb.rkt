...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(cons (or 'bind 'bind*) more)
     (match sexpr
       [(list binder (list bindings ...) body)
         (: get-name : Sexpr -> Symbol)
         (define (get-name binding)
           (match binding
             [(list (symbol: name) named) name]
             [else (error 'parse-sexpr "bad `bind syntax")]))
         (: get-named : Sexpr -> BRANG)
         (define (get-named binding)
           (match binding
             [(list (symbol: name) named) (parse-sexpr named)]
             [else (error 'parse-sexpr "bad `bind syntax")]))
         (let ([names (map get-name bindings)]
               [namedList (map get-named bindings)])
               (if (eq? binder 'bind)
                 (Bind names namedList (parse-sexpr body))
                 (Bind* names namedList (parse-sexpr body))))]
...
;; TODO: add tests
;; TODO: Some functions don't have purpose statements.
